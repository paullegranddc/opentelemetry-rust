...
pub trait SpanProcessor: Send + Sync + std::fmt::Debug {
    ...
    /// in the [`FinishedSpan`] passed to `on_end`.
    ///
    /// # Example
    ///
    /// ```rust,ignore
...
mod tests {
    ...
    fn batchspanprocessor_shutdown() {
        ...
        let mut record = FinishedSpan::new(create_test_span("test_span"));

        processor.on_end(&mut record);
        processor.force_flush().unwrap();
        processor.shutdown().unwrap();

        // todo: expect to see errors here. How should we assert this?
        processor.on_end(&mut FinishedSpan::new(create_test_span("after_shutdown_span")));
