...
fn obfuscate_http_auth_url(s: &str) -> Option<String> {
    let uri = hyper::http::Uri::from_maybe_shared(s).ok()?;
    let authority = uri.authority()?;
    let (auth, url) = authority.as_str().split_once('@')?;
    let new_auth = format!("REDACTED_USERNAME:REDACTED_PASSWORD@{url}");
    Ok(hyper::Uri::from_parts(hyper::http::uri::Parts {
        authority: Some(hyper::http::uri::Authority::from_maybe_shared(new_auth).ok()?),
        ..uri.into_parts()
    })
    .ok()?
    .to_string())
